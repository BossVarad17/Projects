...
import { Heart, Battery, Thermometer, Droplets, Zap, Gauge, LucideIcon } from 'lucide-react'; // Import LucideIcon for typing

// Define a type for the possible status values
type HealthStatus = 'Excellent' | 'Good' | 'Fair' | 'Needs Attention';

// Define a type for the health system objects
interface HealthSystem {
  name: string;
  icon: LucideIcon; // Use LucideIcon type for the icon component
  score: number;
  status: HealthStatus; // Use the HealthStatus union type
  color: string;
  issues: string[];
  lastMaintenance: string;
}

const healthSystems: HealthSystem[] = [ // Use the HealthSystem type for the array
  {
    name: 'Engine',
    icon: Zap,
    score: 87,
...
const getStatusColor = (status: HealthStatus): string => { // Explicitly type status and return type
  switch (status) {
    case 'Excellent': return 'text-green-400';
    case 'Good': return 'text-blue-400';
    case 'Fair': return 'text-yellow-400';
    case 'Needs Attention': return 'text-orange-400';
    default: return 'text-gray-400'; // Added a default case for safety, though HealthStatus union should cover all expected cases
  }
};
