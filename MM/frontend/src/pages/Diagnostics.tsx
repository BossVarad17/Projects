import React, { useState } from 'react'; // Keep React as it might be needed by the build process
import { motion } from 'framer-motion';
import { AlertTriangle, CheckCircle, XCircle, Search, Filter, LucideProps } from 'lucide-react'; // Import LucideProps for typing
import { ForwardRefExoticComponent, RefAttributes } from 'react'; // Import necessary types

// Define a type for the fault code objects
interface FaultCode {
  code: string;
  description: string;
  severity: 'high' | 'medium' | 'low'; // Use a union type for severity
  system: string;
  timestamp: string;
  solution: string;
}

// Define a type for the severity colors object structure
...
const faultCodes: FaultCode[] = [ // Use the FaultCode type for the array
  {
    code: 'P0302',
...
const severityColors: Record<FaultCode['severity'], SeverityColors> = { // Use Record and the severity union type
